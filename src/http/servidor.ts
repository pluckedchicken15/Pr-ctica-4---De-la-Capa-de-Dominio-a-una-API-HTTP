...
// GET /api/prestamos
app.get('/api/prestamos', async (req: Request, res: Response, next: NextFunction) => {
    try {
...
// POST /api/prestamos
app.post('/api/prestamos', async (req: Request, res: Response, next: NextFunction) => {
    try {
...
app.use((error: Error, req: Request, res: Response, next: NextFunction) => {
    if (error instanceof ValidacionError || error.name === 'ValidacionError') {
        return res.status(400).json({
            ...
            mensaje: (error as ValidacionError).detalles || error.message
        });
    }
